...
由于 new String 会构造一个对象，而不是一个 string 直接量，且new String(x) 如果带参数，那么alert它的时候就会返回 x。所以 yx01 将返回 new String(“圆心”) 这个对象，而 alert yx01 则显示 “圆心”。
    ...
    CD.create = function(id, mode) {
        ...
        iframeDoc.designMode = "On";    //想让iframe可以编辑 这个是必须打开 且必须在window完全加载的情况下 设置此属性
    ...
    CD.func = {
        getIframeDoc : function(iframe) {       //获取frame的doc信息
            /*
                【contentDocument】
        ...
        getFullHtml : function(id) {    //获取iframe内部的html结构
            var html = '<html>';
            html += '<head>';
    ...
    CD.toolbar = {
        create : function(id) {
            ...
            for (var i = 0,length=CD.lang["items"].length; i < length; i++) {
                if(CD.lang["items"][i][0] == "picture" && CD.g[id].use != true) continue;
