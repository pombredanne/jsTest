        ...
        getSelection: function(opts) {
            ...
            var dom = this.jquery ? this[0] : this,
                ...
                getFirstStr=function(dom){
                        var first=0,
                            firstDom=dom.firstChild||{data:dom.value};
                        if(firstDom.data.search(/\s/)>-1){    //为了防止eg:<TextNode textContent="\n   this is a test\n   ">]  nodeType=3   <div>  nodeType=1  注释：8
                            ...
                            first=firstDom.textContent.search($.trim(firstDom.textContent).slice(0,1))||0;  //TODO 让得出结果减去它即可
                        }
                };

            return (
                (dom.selectionStart && function() {  // dom 3.0
                    var length = dom.selectionEnd - dom.selectionStart,
                        first=opts.trim?getFirstStr(dom)||0;
                    return { start: dom.selectionStart-first, end: dom.selectionEnd-first, length: length, text: dom.value.substr(dom.selectionStart, l) };
                        ...
                        if(window.getSelection) { //非IE浏览器||>ie8
                            ...
                            if(getSel.anchorNode.textContent === eleText) {
                                ...
                                return {start:start-(first-0),end:result.end(start),value:result.value(),length:result.length(),trim:first};
                            } else {
                                ...
                                return {start:start,end:result.end(start),value:result.value(),length:result.length(),trim:first};
                            }
                        }else{  //<ie9的
                            dom.focus();
                            ...
                            return {start:CaretPos-first,end:end,value:text,length:text.length,trim:first};
                        }
                    })||
                    (document.selection && function() {     //针对<ie9
                        ...
                        if(r == null) {
                            return { start: 0, end: dom.value.length-first,text:'', length: 0 }
                        }
                        ...
                        return { start: rc.text.length-start, end: rc.text.length + r.text.length-first, length: r.text.length, text: r.text };
                    })||
                    function() {  //browser not supported
                        return { start: 0, end: dom.value.length, length: 0 };
                    }
                )();
        },
        ...
        setCursorPosition:function(pos,end,start){
            ...
            if (dom.setSelectionRange) {
                ...
                if(start||end){
                    ...
                    _first=0;
                    var getFirstNode=function(first){
                                    ...
                                    for(var i=0,len=dom.childNodes.length;i<len;i++){
                                        ...
                                        if(cache.childNodes.length){
                                            ...
                                            while(true){
                                                ...
                                                var data=$.trim(Ccache.data||(Ccache.firstChild.data?Ccache.firstChild.data:Ccache.firstChild.textContent));
