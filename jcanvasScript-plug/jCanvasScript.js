...
(function (window, undefined) {
    ...
    var canvases = [],
        ...
        FireFox_lt_7 =FireFox?(parseInt(FireFox[0].split(/[ \/\.]/i)[1]) < 7):false,
        radian = 180 / m_pi,
        m_max = m.max,
        m_min = m.min,
        m_cos = m.cos,
        ...
        findByName=function (i, j, myGroup, stroke) {
            var objs = canvases[i].layers[j].objs,
                grdntsnptrns = canvases[i].layers[j].grdntsnptrns,
        ...
        findByCanvasAndLayer=function (i, j, myGroup) {
            var objs = canvases[i].layers[j].objs,
                grdntsnptrns = canvases[i].layers[j].grdntsnptrns,
                objsLength = objs.length,
                grdntsnptrnsLength = grdntsnptrns.length;
        ...
        changeMatrix=function (object){  //改变矩阵
        ...
        checkDefaults=function (check,def){
            for(var key in def){
        ...
        animating=function (canvasOptns){
        ...
        animateTransforms=function (key,object,queue){
            var val=object[key];
        ...
        mouseEvent=function (e,key,optns){
        ...
        setKeyEvent=function (fn,eventName){
    ...
    proto.object=function(){    // 绘制公用部分
        ...
        this.shadow= function(options){  //绘制阴影
    ...
    var jCanvaScript = function (stroke, map) {
        ...
        var canvasNumber = -1, layerNumber = -1, limitC = canvases.length, myGroup = group(), i, j, canvas, layer, layers, element, limitL,type=stroke&&stroke.charAt(0)||false;
        if (map === undefined) {
            if(type){
                for (i = 0; i < limitC; i++) {
            ...
            if (layerNumber < 0 && canvasNumber < 0){
                return group();
            }
            if (layerNumber < 0) {
    ...
    jCanvaScript.canvas = function(idCanvas){   // 创建canvas 对象的地方
        if(!idCanvas){
            return canvases[0];
        }
        ...
        var canvas={
            id:function(id){
                if(!id)return this.optns.id;
                this.optns.id=id;
                return this;
            }
        };
        canvases[limit]=canvas;  //存储canvas对象
        lastCanvas=limit;
        canvas.cnv=document.getElementById(idCanvas);
        // 获取 getContext('2d')
        if ('\v'=='v'){  // >ie9 浏览器 使用excanvas.js
