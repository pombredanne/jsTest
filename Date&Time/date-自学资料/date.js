//view-source:http://www.jsfoot.com/jquery/demo/2012-03-19/449.html
...
(function() {
    ...
    b("isLeapYear",function() { //闰年
        var c = this.getFullYear();
        return (c % 4 == 0 && c % 100 != 0) || c % 400 == 0
    });

    b("isWeekend",function() {  //是否假日
        return this.getDay() == 0 || this.getDay() == 6
    });

    b("isWeekDay",function() {  //是否工作日
        return ! this.isWeekend()
    });

    b("getDaysInMonth",function() {     //查看这个月有几天 注意:我们认为的月份=他里面的月份+1;
        return [31, (this.isLeapYear() ? 29 : 28), 31, 30, 31, 30, 31, 31, 30, 31, 30, 31][this.getMonth()]
    });

    b("getDayName",function(c) {    //获取中文星期几
        return c ? Date.abbrDayNames[this.getDay()] : Date.dayNames[this.getDay()]
    });
    b("getMonthName",  function(c) {     //获取中文月份   '一月'
        return c ? Date.abbrMonthNames[this.getMonth()] : Date.monthNames[this.getMonth()]
    });
    
    b("getDayOfYear", function() {       //今天距离今年是第几天
        var c = new Date("1/1/" + this.getFullYear());
        return Math.floor((this.getTime() - c.getTime()) / 86400000)
    });
    b("getWeekOfYear",function() {      //今天距离今年是第几周
        return Math.ceil(this.getDayOfYear() / 7)
    });
    b("setDayOfYear",function(c) {      //设置从今年元旦开始的某一天
        this.setMonth(0);
        this.setDate(c);
        return this
    });

    /*   在今年的基础上 添加年/月/日 一定要是number类型 */
    ...
    b('dateAdd',function(date,date1,type){
            var one=date.split('-'),two=date1.split('-');
            type={'day':2,'year':0,'month':1}[type] || 0;
            return date1[type]-date[type]
    });

    b("zeroTime",function() {       //清空今天的时间  只返回今天的日期
    ...
    b("asString",function(d) {      //返回指定格式的日期格式 String  eg:'2013-01-18'
        var c = d || Date.format;
...
(function(d) {
    ...
    d.extend(a.prototype, {
        ...
        display: function(e) {
            ...
            if (this.displayClose) {
                console.log(d.dpText.TEXT_CLOSE)
    ...
    d.fn.datePicker.defaults = {
        ...
        closeOnSelect: true,
        displayClose: false,    //关闭
        selectMultiple: false,  //日期是否可以多选
        numSelectable: Number.MAX_VALUE,
        clickInput: false,  //点击input出发控件
        rememberViewedMonth: false,  //是否记忆点击过的时间
        selectWeek: false,      //给整个一行绑定hover事件
